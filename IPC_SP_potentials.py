...
class Mixin:
    ...
    def print_potential(self, name):
        ...
        if False: #self.npatch == 2 and self.default_topo:
            #name = '2patch_analytic'
            ...
            out = np.reshape((np.asarray(out)),(int(len(out)/2),2))
            ...
            coord = [[ix,iy,iz] for ix in [self.sigma_core] for iy in np.linspace(0,np.pi,50) for iz in np.linspace(0,2.*np.pi,100) ] ##np.linspace(0,2.*np.pi,100) ]
            
            out = []

            for r, th, ph in coord:
                ##out0 = self.analytic_funcdict[name](r, th, ph)
                out0 = self.potential_outside_dd(r, th, ph, real_units=True)               
    ...
    def print_potential_lookup(self, name):
        ...
        for r, th, ph in coord:
           ...
           out0 = self.potential_outside_dd(r, th, ph, real_units=False)               
    ...
    def potential_inside_dd(self, r, th, ph, real_units=False):
    ...
    def potential_outside_dd(self, r, th, ph, real_units=False):
